...
- Parameter recovery: MAE/RMSE/Corr (items_posterior_T)
- Classification: exact/within1 (items_posterior_T)
- Predictive: Brier/LogLoss on held-out test (items_posterior_T)
...
- items_posterior_T.csv
- items_posterior_Txxx.csv (per T)
- review_lists/review_Txxx.csv (optional; if --export_review_lists)

Example runs:

1) รันแบบอ่านไฟล์ CSV
python .\bot_bayes_cefr_mode_1.py --out_dir .\out_csv --bank_csv .\question_bank_180_balanced.csv --export_review_lists

2) รันแบบ Simulate
python .\bot_bayes_cefr_mode_1.py --out_dir .\out_sim --export_review_lists
...
python .\bot_bayes_cefr_mode_1.py `
  ...
  4) ถ้าจะ Simulate แบบกำหนดค่าครบ
...
5) ถ้าใช้ Command Prompt (cmd) ให้เขียนบรรทัดเดียว
python bot_bayes_cefr_mode_1.py --out_dir out_csv --bank_csv question_bank_180_balanced.csv --export_re
"""
...
def main():
    ...
    df_sum.to_csv(os.path.join(args.out_dir, "items_posterior_T.csv"),
                  index=False, encoding="utf-8-sig")
    ...
    print("       items_posterior_T.csv, items_posterior_Txxx.csv, cefr_bands_used.csv (no plots)")
